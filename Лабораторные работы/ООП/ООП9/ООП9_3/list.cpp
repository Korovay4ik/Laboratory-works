...
List::List(int size)
{
	...
	if (size < 0) throw ErrorSizeIsTooSmall();
	if (size > Max_size) throw ErrorSizeIsTooBig();
	if (size > 0)
	{
...
{
	if (index < 0) throw ErrorIndexIsTooSmall();
	if (index > size - 1) throw ErrorIndexIsTooBig();
	Node* current_node = this->head_node;
